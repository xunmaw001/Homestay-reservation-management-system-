package com.entity.view;

import com.entity.CanyinEntity;
import com.baomidou.mybatisplus.annotations.TableName;
import org.apache.commons.beanutils.BeanUtils;
import java.lang.reflect.InvocationTargetException;
import org.springframework.format.annotation.DateTimeFormat;
import com.fasterxml.jackson.annotation.JsonFormat;
import java.io.Serializable;
import java.util.Date;

/**
 * 餐饮
 * 后端返回视图实体辅助类
 * （通常后端关联的表或者自定义的字段需要返回使用）
 */
@TableName("canyin")
public class CanyinView extends CanyinEntity implements Serializable {
    private static final long serialVersionUID = 1L;

		/**
		* 餐饮类型的值
		*/
		private String canyinValue;
		/**
		* 是否上架的值
		*/
		private String shangxiaValue;



	public CanyinView() {

	}

	public CanyinView(CanyinEntity canyinEntity) {
		try {
			BeanUtils.copyProperties(this, canyinEntity);
		} catch (IllegalAccessException | InvocationTargetException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	}



			/**
			* 获取： 餐饮类型的值
			*/
			public String getCanyinValue() {
				return canyinValue;
			}
			/**
			* 设置： 餐饮类型的值
			*/
			public void setCanyinValue(String canyinValue) {
				this.canyinValue = canyinValue;
			}
			/**
			* 获取： 是否上架的值
			*/
			public String getShangxiaValue() {
				return shangxiaValue;
			}
			/**
			* 设置： 是否上架的值
			*/
			public void setShangxiaValue(String shangxiaValue) {
				this.shangxiaValue = shangxiaValue;
			}


























}
