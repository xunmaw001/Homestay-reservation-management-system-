    ...
    */
    ...
    public R save(@RequestBody TechanLiuyanEntity techanLiuyan, HttpServletRequest request){
        ...
        if(false)
            ...
            techanLiuyan.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
...
//            techanLiuyan.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        //根据字段查询是否有相同数据
        Wrapper<TechanLiuyanEntity> queryWrapper = new EntityWrapper<TechanLiuyanEntity>()
            .eq("id",0)
            ;

        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        TechanLiuyanEntity techanLiuyanEntity = techanLiuyanService.selectOne(queryWrapper);
        techanLiuyan.setUpdateTime(new Date());
        if(techanLiuyanEntity==null){
    ...
    */
    ...
    public R save( String fileName, HttpServletRequest request){
        ...
        try {
            List<TechanLiuyanEntity> techanLiuyanList = new ArrayList<>();//上传的东西
                ...
                if(!".xls".equals(suffix)){
                    ...
                    if(!file.exists()){
                        ...
                        for(List<String> data:dataList){
                            //循环
                            TechanLiuyanEntity techanLiuyanEntity = new TechanLiuyanEntity();
//                            techanLiuyanEntity.setTechanId(Integer.valueOf(data.get(0)));   //特产 要改的
//                            techanLiuyanEntity.setYonghuId(Integer.valueOf(data.get(0)));   //用户 要改的
//                            techanLiuyanEntity.setTechanLiuyanText(data.get(0));                    //留言内容 要改的
//                            techanLiuyanEntity.setInsertTime(date);//时间
//                            techanLiuyanEntity.setReplyText(data.get(0));                    //回复内容 要改的
//                            techanLiuyanEntity.setUpdateTime(sdf.parse(data.get(0)));          //回复时间 要改的
//                            techanLiuyanEntity.setCreateTime(date);//时间
                            techanLiuyanList.add(techanLiuyanEntity);
                        ...
                        //查询是否重复
                        techanLiuyanService.insertBatch(techanLiuyanList);
                        return R.ok();
                    }
                }
            }
