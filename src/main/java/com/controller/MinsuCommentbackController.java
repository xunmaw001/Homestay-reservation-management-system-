...
public class MinsuCommentbackController {
    ...
    */
    ...
    public R update(@RequestBody MinsuCommentbackEntity minsuCommentback, HttpServletRequest request){
...
//            minsuCommentback.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        //根据字段查询是否有相同数据
        Wrapper<MinsuCommentbackEntity> queryWrapper = new EntityWrapper<MinsuCommentbackEntity>()
            .eq("id",0)
            ;

        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        MinsuCommentbackEntity minsuCommentbackEntity = minsuCommentbackService.selectOne(queryWrapper);
        minsuCommentback.setUpdateTime(new Date());
        if(minsuCommentbackEntity==null){
